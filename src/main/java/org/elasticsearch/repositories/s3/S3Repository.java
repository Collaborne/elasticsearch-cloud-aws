...
public class S3Repository extends BlobStoreRepository {
    ...
    public S3Repository(RepositoryName name, RepositorySettings repositorySettings, IndexShardRepository indexShardRepository, AwsS3Service s3Service) throws IOException {
        ...
        Boolean pathStyleAccess = repositorySettings.settings().getAsBoolean("path_style_access", componentSettings.getAsBoolean("path_style_access", null));
        ...
        logger.debug("using bucket [{}], region [{}], endpoint [{}], protocol [{}], chunk_size [{}], server_side_encryption [{}], path_style_access [{}], buffer_size [{}], max_retries [{}]",
                bucket, region, endpoint, protocol, chunkSize, serverSideEncryption, pathStyleAccess, bufferSize, maxRetries);

        blobStore = new S3BlobStore(settings, s3Service.client(endpoint, protocol, region, repositorySettings.settings().get("access_key"), repositorySettings.settings().get("secret_key"), maxRetries, pathStyleAccess), bucket, region, serverSideEncryption, bufferSize, maxRetries);
